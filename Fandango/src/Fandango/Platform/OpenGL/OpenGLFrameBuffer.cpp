...
namespace Fandango
{
	...
	OpenGLFrameBuffer::OpenGLFrameBuffer(const FrameBufferSpec& spec)
		: m_Spec(spec)
	{
		for (auto format : m_Spec.AttachmentSpec.Attachments)
		{
	...
	void OpenGLFrameBuffer::Recreate()
	{
		...
		{
			FNDG_ENGINE_ASSERT(m_ColorAttachments.size() <= 4);
	...
	{
		FNDG_ENGINE_ASSERT(attachmentIndex < m_ColorAttachments.size());

		// Note: this assumes the FrameBuffer is already bount
		int pixelData;
